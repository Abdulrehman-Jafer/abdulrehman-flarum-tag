...
namespace Abdulrehman\Tags\Tests\integration\api\discussions;
...
class ListTest extends TestCase
{
    ...
    {
        if ($type === 'secondary') {
            $this->setting('abdulrehman-tags.min_secondary_tags', 1);
            $this->database()->table('tags')
                ->where(['position' => null, 'parent_id' => null])
                ->update(['position' => 200]);
        } elseif ($type === 'primary') {
            $this->setting('abdulrehman-tags.min_primary_tags', 100);
            $this->database()->table('tags')
                ->where('position', '!=', null)
